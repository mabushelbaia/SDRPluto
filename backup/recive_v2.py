...
sample_rate = 1e6  # Hz
center_freq_tx = (
    int(input("center_freq transmiter in Hz = ")) * sample_rate
)  # 1000e6 # Hz
# center_freq_rx = (
#     int(input("center_freq receiver in Hz = ")) * sample_rate
# )  # 1000e6 # Hz
...
sdr.tx_hardwaregain_chan0 = float(
    input("tx power (valid range is -90 to 0 dB) = ")
)  # 0.0 Increase to increase tx power, valid range is -90 to 0 dB
...
# sdr.rx_hardwaregain_chan0 = float(
#     input("receive gain (be careful not to saturate the ADC) = ")
#)  # 10.0 # dB, increase to increase the receive gain, but be careful not to saturate the ADC

### Transmit Sinewave
fc = int(input("transmited carrier in Hz = "))  # 2000
...
while True:
    ...
    #  plt.plot(t,np.real(tx_samples - rx_samples))
    #  plt.ylabel("Amplitude")
    #  plt.title('noise')
    #  plt.xlabel("Time")

    # if the figure is closed, exit out of the loop
